...
public class ChargeEventHandler {
  ...
  public static void onAttachCapabilitiesEvent(AttachCapabilitiesEvent<Entity> event) {
    if (event.getObject() instanceof CreatureEntity) {
      ...
      event.addCapability(new ResourceLocation(DababyMod.MODID, "health"), provider);
  ...
  public static void onDeathEvent(LivingDeathEvent event) {
    ...
    entity
        .getCapability(CapabilityEntityCharge.ENTITY_CHARGE_CAPABILITY)
        .ifPresent(
            h -> {
                ...
                h -> {
                  ...
                  player.displayClientMessage(
                      new TranslationTextComponent(
                          "message.increase_charge", Integer.toString(charge)),
                      true);
                  stack.shrink(1);
